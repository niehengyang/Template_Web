        ...
        >
            ...
            >
                ...
                <el-form-item :label="$t('lang.show-name')" prop="df_displayname">
                    <el-input
                        size="medium"
                        v-model="createForm.df_displayname"
                        onkeyup="this.value=this.value.replace(/\s+/g,'')"
                ...
                <el-form-item
                    ...
                    required
                    v-if="createForm.df_type!='00' && createForm.df_type"
                >
                    <!-- 新数据配置列表 -->
                    <el-row
                        v-for="(item,index) in createForm.df_datamap"
                        :key="index"
                        :gutter="20"
                ...
                <el-form-item :label="$t('lang.unit')" v-if="createForm.df_type == '00'" prop="df_unit">
                    <el-input size="medium" v-model="createForm.df_unit" :placeholder="$t('lang.hint-unit')"></el-input>
                </el-form-item>
            </el-form>
    ...
    methods: {
        ...
        create() {
            this.$refs.createForm.validate(valid => {
                ...
                this.$api.restfulApi
                    .create("dataflow", this.createForm)
                    .then(() => {
                        this.loading = false;
                        this.$message({
        ...
        chooseDataType(val) {
            console.log(val);
            if (val != "00") {
                this.createForm.df_unit = "";
            }
            this.createForm.df_datamap = [
                {
                    key: "",
                    value: ""
                }
        ...
        //增加配置行
        addNewConfigRow() {
            this.createForm.df_datamap.push({
                key: "",
                value: ""
            });
        },
        ...
        deleteConfigRow(index, item) {
            this.index = this.createForm.df_datamap.indexOf(item);
            if (index !== 0) {
                this.createForm.df_datamap.splice(index, 1);
            }
        },
        //关闭
        close() {
