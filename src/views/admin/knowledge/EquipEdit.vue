<template>
    <div>
        ...
        <div slot="footer" class="dialog-footer dialog-btn-group" style="text-align: right;">
            <el-button class="white-btn" size="medium" @click="closeDialog('editForm')" :disabled="allotLoading?true:false">{{$t('lang.cancel')}}</el-button>
            <el-button class="blue-btn" size="medium" type="primary" @click="edit('editForm')" :disabled="allotLoading?true:false">{{$t('lang.submit')}}</el-button>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            initEdit(){
                ...
                this.$api.restfulApi.item('devicefile', this.editData.id).then(res => {
                    console.log('我是res',res)
                    res.xmbadmins.airConUser.forEach(item => {
                        this.allotData.push(['2', item])
                    })
            ...
            edit() {
                ...
                this.$refs.editForm.validate(valid => {
                    ...
                    this.editLoading = true;
                    this.$api.restfulApi
                        .edit("devicefile", this.editForm.id, this.editForm)
                        .then(res => {
