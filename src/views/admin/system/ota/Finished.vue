            ...
            <el-row class="table-row" v-loading="tableLoading">
                <el-col :span="24">
                    <el-table :data="tableData" style="width: 100%">
                        ...
                        <el-table-column prop="device.serialno" :label="$t('lang.device-serial')"></el-table-column>
                        <el-table-column prop="device.deviceattach.name" :label="$t('lang.device-name')"></el-table-column>
                        <el-table-column :label="$t('lang.mac-address')" prop="mac"></el-table-column>
                        <el-table-column prop="device.type.name" :label="$t('lang.device-type')"></el-table-column>
                        <el-table-column :label="$t('lang.current-version')">
                            <template slot-scope="scope">
                                <span v-if="scope.row.version == null">--</span>
                                <span v-else>{{scope.row.version}}</span>
                            </template>
                        </el-table-column>
                        <el-table-column prop="target_version" :label="$t('lang.target-version')">
                            <template slot-scope="scope">
                                <span v-if="scope.row.version_upgrade == null">--</span>
                                <span v-else>{{scope.row.version_upgrade}}</span>
                            </template>
                        </el-table-column>
